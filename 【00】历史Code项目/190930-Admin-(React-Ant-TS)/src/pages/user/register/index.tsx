...
class Register extends Component<RegisterProps, RegisterState> {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'userRegister/fetchCommunities',
    });
    dispatch({
      type: 'userRegister/fetchMerchants',
    });
  }
  ...
  checkPassword = (rule: any, value: string, callback: (messgae?: string) => void) => {
    ...
    if (!value) {
      ...
      if (value.length < 6 || !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,10}$/.test(value)) {
        callback('error');
      } else {
        const { form } = this.props;
  ...
  render() {
    const { userRegister, form, submitting } = this.props;
    const { communities, merchants } = userRegister
      ...
      <div className={styles.main}>
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem>
            ...
            })(<Input size="large" placeholder="商户名称" />)}
          </FormItem>
          <FormItem>
            ...
            })(<Input size="large" placeholder="商户地址" />)}
          </FormItem>
          <FormItem>
          ...
          <FormItem>
            ...
            })(<Input size="large" placeholder="负责人" />)}
          </FormItem>
          <FormItem>
            {getFieldDecorator('SubscribePhone', {
              rules: [
                {
                  required: true,
                  message: '请输入预约电话！',
